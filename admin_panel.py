...
div.stButton>button {
    background: linear-gradient(135deg,#2563eb,#38bdf8); color:#020617;
    border-radius:12px; height:46px; font-weight:700; border:none;
...
.metric {
    ...
    border:1px solid #1e293b; padding:22px; border-radius:18px;
    color:#e5e7eb; text-align:center; font-weight:700;
...
INTENTS_FILE = f"{DATA}/intents_clean.json"
KB_FILE = f"{DATA}/knowledgebase_clean.json"
...
# ---------------- INTENT MANAGER ----------------
    ...
    if st.button("Add Intent"):
        if tag.strip() and patterns.strip() and responses.strip():
            intents.append({"tag":tag.strip(),"patterns":patterns.lower().splitlines(),"responses":responses.splitlines()})
    ...
    if intents:
        ...
        selected_intent = next((i for i in intents if i["tag"]==sel), None)
        if selected_intent:
...
# ---------------- KNOWLEDGE BASE ----------------
    ...
    if st.button("Add Knowledge"):
        if q.strip() and a.strip():  # Ensure non-empty
            kb.append({"question": q.strip(), "answer": a.strip()})
    ...
    if kb:  # Only show if there are articles
        st.markdown("### Existing Articles")
        # Create a selectbox for questions
        sel_question = st.selectbox(
            "Select Article",
            [k.get("question", "Unknown Question") for k in kb]
        )
        # Show answer of selected question
        selected_article = next((k for k in kb if k.get("question") == sel_question), None)
        if selected_article:
            st.info(selected_article.get("answer", "No answer found"))
...
# ---------------- CHATBOT TEST ----------------
    ...
    if query.strip():
        ...
        analytics = load_json(ANALYTICS_FILE)
        analytics.append({
            "intent":df_scores.iloc[0]["Intent"],
            "confidence":float(df_scores.iloc[0]["Confidence"]),
            "query":query,
...
# ---------------- ANALYTICS ----------------
    ...
    df["time"] = pd.to_datetime(df["time"], errors='coerce')  # safe conversion
    ...
    c1, c2 = st.columns(2)
    c1.altair_chart(
        alt.Chart(d).mark_line(point=True).encode(x="time:T",y="confidence:Q").interactive(),
        use_container_width=True
    )

    donut = pd.DataFrame({"Category":["High ≥75%","Low <75%"],"Count":[len(d[d.confidence>=0.75]),len(d[d.confidence<0.75])]})
    c2.altair_chart(
        alt.Chart(donut).mark_arc(innerRadius=60).encode(theta="Count",color="Category:N").interactive(),
        use_container_width=True
    )
    ...
    for intent in unique_intents:
        ...
        overview.append({
            "Intent":intent,
            "High ≥75%":len(d_int[d_int.confidence>=0.75]),
            "Low <75%":len(d_int[d_int.confidence<0.75])
        })
    ...
    st.altair_chart(
        alt.Chart(stacked_df).mark_bar().encode(
            x="Intent:N",y="Count:Q",color="Category:N",tooltip=["Intent","Category","Count"]
        ),use_container_width=True
    )
